...
export function createStarmap(onSelectSystem) {
  ...
  function redraw() {
    ...
    const sensorRange = effectiveSensorRange(save);
    ...
    // right now would reveal. Rendered as a soft filled disc so it reads as a
    // "cleared" patch against the dark backdrop, distinct from the fuel-range
    // ring below.
    viewport.appendChild(svgEl('circle', {
